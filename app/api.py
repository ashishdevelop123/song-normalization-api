...
from app.models import Song, PaginatedSongs
import os

router = APIRouter()

DATA_PATH = os.path.join("data", "sample.json")
songs_data: list[Song] = normalize_json(DATA_PATH)
...
def get_all_songs(cursor: Optional[str] = None, limit: int = 10):
    ...
    return PaginatedSongs(items=paginated, next_cursor=next_cursor, has_more=has_more)


@router.get("/songs/by_title", response_model=Song)
...
@router.post("/songs/{song_id}/rate", response_model=Song)
